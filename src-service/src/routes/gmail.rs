...
pub fn router() -> Router<ApiState> {
    Router::new()
        .route("/gmail/connect",    post(connect_gmail))
        .route("/gmail/disconnect", post(disconnect_gmail))
        .route("/gmail/status",     get(gmail_status))
}

/// POST /gmail/connect
/// Body: { "refresh_token": "...", "client_id": "..." (optional) }
async fn connect_gmail(
    ...
    let ok = crate::service::harness::tools::gmail_tools::store_gmail_refresh_token(
    ...
    if ok {
        ...
        Err((StatusCode::INTERNAL_SERVER_ERROR, "Failed to store Gmail token".to_string()))
    }
}
...
async fn disconnect_gmail(
    ...
    let _ = state.db
        .query("UPDATE user_settings SET `value` = '', updated_at = $now \
                WHERE username = $u AND `key` = 'google_gmail_refresh_token'")
        .bind(("u", account_id))
        .bind(("now", now))
        .await;
    Ok(Json(json!({ "ok": true })))
...
async fn gmail_status(
    ...
    let mut r = state.db
        .query("SELECT `value` FROM user_settings WHERE username = $u AND `key` = 'google_gmail_refresh_token' LIMIT 1")
        .bind(("u", account_id))
        .await
